  ...
  return (
    <div className="min-h-screen flex flex-col font-sans text-stone-800 bg-stone-50 selection:bg-stone-900 selection:text-white">
      ...
      <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 
          ${isScrolled || !isHomePage ? 'bg-white/95 backdrop-blur-md shadow-sm py-4' : 'bg-transparent py-6'}
      `}>
          <div className="max-w-7xl mx-auto px-6 lg:px-12 flex items-center justify-between gap-4">
              ...
              <div ref={searchRef} className="hidden md:flex flex-1 max-w-md mx-auto relative group flex-col">
                  <form onSubmit={handleSearch} className="relative w-full z-50">
                    <input 
                        ...
                        className={`w-full border border-transparent rounded-full py-2.5 pl-5 pr-12 text-sm outline-none transition-all duration-300 placeholder:text-stone-400 z-50 relative
                            ${isScrolled || !isHomePage ? 'bg-stone-100 focus:bg-white focus:border-stone-200' : 'bg-white/80 focus:bg-white focus:border-white shadow-lg'}
                        `}
                    />
                    <button 
                        type="submit"
              ...
              <div className="flex items-center gap-6 shrink-0">
                  {/* Desktop Nav Links */}
                  <nav className={`hidden lg:flex items-center gap-6 text-sm font-medium transition-colors ${!isHomePage || isScrolled ? 'text-stone-600' : 'text-stone-800'}`}>
                  ...
                  <div className="flex items-center gap-4">
                      {/* Wishlist */}
                      <Link to="/wishlist" className={`relative p-2 transition-colors ${!isHomePage || isScrolled ? 'text-stone-800 hover:text-stone-600' : 'text-stone-900 hover:text-stone-700'}`}>
                      ...
                      {/* Cart */}
                      <Link to="/cart" className={`relative p-2 transition-colors ${!isHomePage || isScrolled ? 'text-stone-800 hover:text-stone-600' : 'text-stone-900 hover:text-stone-700'}`}>
                          <ShoppingBag className="w-5 h-5" />
                          {cartItems.length > 0 && (
                      ...
                      {user ? (
                          ...
                          <Link to="/login" className={`text-sm font-bold transition-colors ${!isHomePage || isScrolled ? 'text-stone-900 hover:text-stone-600' : 'text-stone-900 hover:text-stone-700'}`}>
                              Login
                          </Link>
                      )}
